...
class RCAAnalyzer:
    ...
    def _should_run(self, result: UnifiedResult) -> bool:
        """Only run RCA when there are actual error signals to investigate."""
        ...
        return has_log_errors or has_error_correlations or has_error_traces
    ...
    def _build_prompt(self, result: UnifiedResult) -> str:
        ...
        error_lines = [
            ...
            if l.severity in (Severity.ERROR, Severity.CRITICAL)
        ]
        if error_lines:
            lines.append(f"## Error log samples ({len(error_lines)} total errors)")
            for log in error_lines[-MAX_LOG_SAMPLES:]:
                ts = log.timestamp.strftime("%H:%M:%S")
                lines.append(f"  [{ts}] {log.message[:200]}")
            lines.append("")
  ...
  "github_search_terms": [
    "<function name, class name, error string, or file path to search for in GitHub>",
    ...
  ]
}""")
